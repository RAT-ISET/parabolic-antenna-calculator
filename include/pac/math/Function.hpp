...
namespace Functions
{
        ...
        (const size_t want, const ParameterList& list)
    {
        if (want == 1)
        {
            ...
            if (!out.has_value()) return unexpected(out.error());
            logger.addEntry(out.value());
            return list[3].value() * numbers::pi * sqrt(list[2].value() / list[4].value());
        }
        if (want == 2) {
            ...
            if (!out.has_value()) return unexpected(out.error());
            logger.addEntry(out.value());
            return list[4].value() * sq(list[1].value() / (numbers::pi * list[3].value()));
        }
        if (want == 3) {
            ...
            if (!out.has_value()) return unexpected(out.error());
            logger.addEntry(out.value());
            return list[1].value() * Constants::BACK_PI * sqrt(list[4].value() / list[2].value());
        }
        if (want == 4) {
            ...
            if (!out.has_value()) return unexpected(out.error());
            logger.addEntry(out.value());
            return list[2].value() * sq(numbers::pi * list[3].value() / list[1].value());
        }
        return unexpected(UNMATCHED_INPUT_PARAMETER);
    }
}
